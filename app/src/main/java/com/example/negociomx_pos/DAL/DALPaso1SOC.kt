...
class DALPaso1SOC {
    ...
    suspend fun obtenerEstadisticasPorFecha(fecha: String): Map<String, Int> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val query = """
                SELECT 
                    ...
                    SUM((SELECT COUNT(*) FROM Paso1LogVehiculoFotos pf WHERE pf.IdPaso1LogVehiculo = p.IdPaso1LogVehiculo)) AS TotalFotos
                FROM dbo.Paso1LogVehiculo p
                WHERE DATEPART(YEAR, p.FechaAlta) = ?
                  AND DATEPART(MONTH, p.FechaAlta) = ?
                  AND DATEPART(DAY, p.FechaAlta) = ?
            ...
            Log.e("DALConsultaPaso1SOC", "Error obteniendo estadísticas: ${e.message}")
        } finally {
            resultSet?.close()
            statement?.close()
            conexion?.close()
